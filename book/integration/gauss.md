...
В разделе {ref}`chapters:integration:newton-cotes` мы видели, что веса квадратуры с известными 
узлами $\{x_i\}$ могут быть найдены решением линейной системы уравнений

```{math}
\sum\limits_{i=1}^{N} w_i x_i^p = \frac{1}{p + 1} \quad\forall p \in \{0, \dots, N-1\}.
```

Хотя она была записана для формул Ньютона-Котса, очевидно, что её можно 
решить относительно весов для любого набора узлов $\{x_i\}$, если они различны.

Но эти же условия можно рассматривать как условия на *веса и узлы* квадратуры, т.е. рассмотреть 
...
Поскольку в таком случае мы имеем $2N$ степеней свободы, можно надеяться, что система будет разрешима 
вплоть до $M = 2N-1$. Если это так, то мы получим квадратурную формулу с $N$ узлами, точную для 
многочленов вплоть до степени $2N-1$.

Решение системы "в лоб" из-за её нелинейности проблематично, но оказывается, что задача имеет весьма 
элегантное решение.

Рассмотрим интегрирование функций на отрезке $[-1, 1]$. Введём для квадратурной формулы с узлами 
$\{x_1, x_2, \dots, x_N\}$ многочлен

```{math}
\Omega_{N}(x) = (x - x_1)(x - x_2)\dots(x - x_N).
```

Произвольный многочлен $P_M(x)$ степени $M \ge N$ можно выразить как 

```{math}
P_M(x) = \Omega_{N}(x) q_{M-N}(x) + r_{N-1}(x),
```

где $q_{M-N}(x)$ -- многочлен степени $M - N$, $r_{N-1}$ -- многочлен степени не выше $N - 1$.

Запишем ошибку интегрирования от $P_M(x)$ при условии, что веса квадратуры определены из {eq}`NewtonCotesGeneral`, 
...
Вторая скобка равна нулю благодаря выбору весов. Сумма в первой скобке равна нулю, т.к. по своему определению 
$\Omega_{N}(x_i) = 0$.

Отсюда получаем вывод: ошибка интегрирования равна нулю, если

```{math}
\int\limits_{-1}^{1} \Omega_{N}(x) q_{M-N}(x) \diff x = 0
```

для всех многочленов степени $M-N$, т.е. многочлен $\Omega_{N}(x)$ ортогонален всем многочленам вплоть до степени $M - N$, 
если скалярное произведение определено как 
...
Т.е. оптимальный выбор узлов квадратуры -- *нули многочленов Лежандра*.

Квадратурные формулы с узлами в нулях многочленов Лежандра называются квадратурами Гаусса и имеют при заданном числе узлов 
(т.е. количестве вычислений подынтегральной функции) максимальный алгебраический порядок точности из всех квадратур вида {eq}`int`.

На практике для узлов и весов квадратур Гаусса различных порядков составлены таблицы, которыми можно пользоваться и не зная 
теории. При использовании таблиц нужно обратить внимание на два аспекта:
