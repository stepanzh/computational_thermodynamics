...
Решение системы "в лоб" из-за её нелинейности проблематично, но оказывается, что задача имеет весьма  элегантное решение.

**Вывод квадратуры Гаусса**

Рассмотрим интегрирование функций на отрезке $[-1, 1]$.
Введём для квадратурной формулы с узлами $\{x_1, x_2, \dots, x_N\}$ многочлен степени $N$ *с нулями в узлах*
...
Значит, в качестве $\Omega_N$ возьмём $L_N$, чтобы удовлетворить {eq}`gauss_orthogonal` для как можно большего числа полиномов $q_{M-N}$ (для остальных же полиномов $L_N$ даёт наименьшее скалярное произведение). Если степень $q_{M-N}$ не превосходит $M-N \le N-1$, т.е. $M \le 2N-1$ тогда {eq}`gauss_orthogonal` выполнится, т.е.
