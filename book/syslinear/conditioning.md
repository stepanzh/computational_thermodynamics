...
По аналогии с обусловленностью вычисления скалярной функции {eq}`condscalar`, вводится понятие обусловленности линейной системы

$$
\mathbf{A} \mathbf{x} = \mathbf{b}.
$$

В {numref}`разделе %s <sec:intro:cond:cond_example>` мы рассматривали пример для обусловленности вычисления $y = f(x)$.
То был пример *прямой* задачи.
Решение же системы линейных уравнений является *обратной* задачей: нам известен прямой оператор $f \to \mathbf{A}$ и его значение $y \to \mathbf{b}$, а найти нужно аргумент оператора $x \to \mathbf{x}$.

Нас интересует, как относительное изменение правой части повлияет на относительное изменение решения.
Т.е. рассмотрим возмущенную систему
